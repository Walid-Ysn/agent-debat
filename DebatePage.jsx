...
export default function DebatePage({ sessionId, navigate }) {
  ...
  const runArbitre = async () => {
    ...
    try {
      ...
      setArguments(prev => [...prev,
        { agent_type: "ARBITRE", round_number: 99, content: res.verdict, id: Date.now() },
      ]);
      setStep("done");
    } catch (e) { setError(e.message); }
    finally { setLoading(false); }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {STEPS.map((s, i) => (
          <div key={s.key} className="flex items-center gap-2 flex-shrink-0">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              i < currentStepIdx  ? "bg-green-900/40 text-green-400 border border-green-800" :
              i === currentStepIdx ? "bg-green-600 text-white shadow shadow-green-900/50" :
      ...
      {error && (
        <div className="bg-red-900/30 border border-red-700 text-red-400 rounded-xl p-4 text-sm mb-6">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <AgentPanel
          ...
          streamContent={streaming.pour}
        />
        <AgentPanel
          type="CONTRE"
          arguments={contreArgs}
          loading={loading && step === "idle"}
          streamContent={streaming.contre}
        />
      </div>
      ...
      {/* Questions panel */}
      {(step === "questions" || step === "done") && (
        <QuestionBox onAsk={handleQuestion} disabled={step === "done"} />
      )}
